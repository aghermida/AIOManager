...
6. **Mark any fork-only code that must stay inside a shared file with a `// [FORK-ONLY: <slug>]` comment**, `<slug>` being a short, unique, descriptive kebab-case name for the feature (not a sequential number). This is deliberately *not* the `9xxxx`-style numeric ID scheme used for DB migration IDs in the sister forks (`AIOStreams`/`aiometadata`) — that numbering exists there because migration IDs are checked against a persisted `_migrations` table in production, so reusing or renumbering one can crash the server on boot (see those repos' `CLAUDE.md`s for the incidents that caused it). Nothing here is a DB migration or has any persisted "applied" state — these are plain source comments with no runtime significance beyond marking a line as fork-owned, so removing one is always as simple as deleting the marked lines; there is no database to reconcile and no ordering to get right. Currently registered:
   - `single-account-limit` — logic lives in `server/fork-single-account-limit.js` (`rejectIfSingleAccountLimitReached`); hooked with a two-line call (import + guard) from `server/index.js`'s `POST /api/sync/:id` handler. Once one account exists in `kv_store`, claiming a *new* id is rejected (updates to the existing id still work).
